...
function (wrap::TorchModuleWrapper)(args...; kwargs...)
    ...
    params = wrap.params
    tensor_out = wrap.torch_stateless_module(Tuple(map(x -> DLPack.share(x, Py, dlpack.from_dlpack).requires_grad_(true), params)),
...
function ChainRulesCore.rrule(wrap::TorchModuleWrapper, args...; kwargs...)
    T = typeof(first(args))
    params = wrap.params
    torch_primal, torch_vjpfun = functorch.vjp(@pyeval("buffer_implicit")(wrap.torch_stateless_module, wrap.buffers), Tuple(map(x -> DLPack.share(x, PyObject, pyfrom_dlpack).requires_grad_(true), params)),
